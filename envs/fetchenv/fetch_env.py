...
class ComplexFetchEnv:
    ...
    def __init__(self, model_file='teleOp_boxes.xml', nsubsteps=20, min_grip_score=0,
        ...
        self.observation_space = gym.spaces.Box(-3., 3., shape=(268 + 336 * self.incl_extra_full_state,), dtype='float32')
        if state_is_pixels:
    ...
    def _get_state(self):
        if self.cached_state is None:
            ...
            #print("self.sim.data contains: ", vars(self.sim.data))
            # print("self.object_names = ", self.object_names)
            # print("self.object_ids = ", self.object_ids)
            # print("self.sim.data.body_xpos[self.object_ids[0]] = ", self.sim.data.body_xpos[self.object_ids[0]])

            log_object_pos = self.sim.data.body_xpos[self.object_ids[0]]

            # print("index of grip_pos = ", list(self._get_full_state()).index(grip_pos[0]))
            # print("index of grip_pos = ", list(self._get_full_state()).index(grip_pos[1]))
            # print("index of grip_pos = ", list(self._get_full_state()).index(grip_pos[2]))
            # print("index of object_pos = ", list(self._get_full_state()).index(log_object_pos[0]))
            # print("index of object_pos = ", list(self._get_full_state()).index(log_object_pos[1]))
            # print("index of object_pos = ", list(self._get_full_state()).index(log_object_pos[2]))
    ...
    def _get_reward(self, prev_state, state):
        # print("prev_state.gripped_info = ", prev_state.gripped_info)
        # if prev_state.gripped_info is not None:
        #     prev_grasping_bool = 1.
        # else:
        #     prev_grasping_bool = 0.
        #
        # if state.gripped_info is not None:
        #     now_grasping_bool = 1.
        # else:
        #     now_grasping_bool = 0.

        # print("prev_grasping_bool = ", prev_grasping_bool)
        # print("now_grasping_bool = ", now_grasping_bool)

        # print("state.object_pos = ", state.object_pos)

        if self.target_location is None:
            # print("prev_state.object_pos = ", prev_state.object_pos)
            obj_prev = np.array(list(map(int, ''.join(prev_state.object_pos))))
            # print("obj_prev = ", obj_prev)
        ...
        # return int(np.sum(obj_now * door_factor * prev_grasping_bool - obj_prev * prev_door_factor * prev_grasping_bool))
    ...
    def _get_extra_full_state(self, include_names=False):
        ...
        for contact in self._iter_contact():
            ...
            if contact in self.contact_indexes:
                # print("self.contact_indexes[contact] = ", self.contact_indexes[contact])
                contacts[self.contact_indexes[contact]] = 1

        # print("contacts[167] = ", contacts[167])
        # print("contacts[193] = ", contacts[193])
        ...
        # print("extra_state[167] = ", extra_state[167])
        # print("extra_state[193] = ", extra_state[193])
    ...
    def _get_full_state(self, include_names=False):
        if self.cached_full_state is None or include_names != isinstance(self.cached_full_state, tuple):
            ...
            # print("len(self.filtered_idxs_for_full_state) =", len(self.filtered_idxs_for_full_state))
            ...
            # ## change for quaternions instead of euler angles (no discontinuities)
            # dim_size = len(self.filtered_idxs_for_full_state) * 3
            ...
            # if self.incl_extra_full_state:
            #     extra_state, extra_names = self._get_extra_full_state(include_names)
            #     all_state = np.empty(len(extra_state) + dim_size * n_dims + dim_size_quat)
            #     all_state[dim_size * n_dims + dim_size_quat:] = extra_state
            # else:
            ...
            # dim_size_quat = len(self.filtered_idxs_for_full_state) * 4
            # #
            # # print("self.sim.model.body_name2id('gripper_link') = ", self.sim.model.body_name2id('gripper_link'))
            # # print("self.filtered_idxs_for_full_state = ", self.filtered_idxs_for_full_state)
            # # print("len(self.filtered_idxs_for_full_state )= ",len(self.filtered_idxs_for_full_state ) )
            #
            # all_state[:dim_size_quat] = quats.flatten()
            # all_state[dim_size_quat:dim_size_quat + dim_size] = self.sim.data.body_xpos[self.filtered_idxs_for_full_state].flatten()
            # all_state[dim_size_quat + dim_size:dim_size_quat + 2*dim_size] = (self.sim.data.body_xvelp[self.filtered_idxs_for_full_state]).flatten()
            # all_state[dim_size_quat + 2*dim_size:dim_size_quat + 3*dim_size] = (self.sim.data.body_xvelr[self.filtered_idxs_for_full_state]).flatten()
            # all_state[dim_size_quat + dim_size:dim_size_quat + 3*dim_size] *= dt
            #
            # print("4*dim_size = ", 4*dim_size)
            # print("dim_size_quat + 3*dim_size = ", dim_size_quat + 3*dim_size)
            # print("all_state.shape = ", all_state.shape)
