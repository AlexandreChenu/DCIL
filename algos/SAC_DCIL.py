...
class SAC(OffPolicyAlgorithm):
	...
	def train(self, gradient_steps: int, batch_size: int = 64) -> None:
		...
		for gradient_step in range(gradient_steps):
			...
			if self.train_iteration > self.warmup_duration and self.add_bonus_reward:# and self.all_skills_feasible:
				...
				next_desired_goals = []
				for info in infos:
					desired_goal_states.append(list(self.env.envs[0].skill_manager.get_goal_state(info[0]["goal_indx"])))
				...
				transformed_rewards = self._transform_rewards_bounded_squashed(replay_data,  infos, replay_data.next_observations, desired_goal_states, desired_goals,  next_desired_goals, overshoot_goal)
	...
	def _transform_rewards_bounded_squashed(self, replay_data, infos, next_obs, true_desired_goal_states, true_desired_goals, shift_desired_goals, overshoot_goal):
		...
		## Compute V(s_0, g')
		goal_state_shift_desired_goal = copy.deepcopy(next_obs)
		goal_state_shift_desired_goal["observation"] = goal_state_shift_desired_goal["observation"].cpu().numpy()
		goal_state_shift_desired_goal["achieved_goal"] = goal_state_shift_desired_goal["achieved_goal"].cpu().numpy()
		goal_state_shift_desired_goal["desired_goal"] = goal_state_shift_desired_goal["desired_goal"].cpu().numpy()
		goal_state_shift_desired_goal["observation"][:] = np.array(true_desired_goal_states)[:]
		goal_state_shift_desired_goal["achieved_goal"][:] = np.array(true_desired_goals)[:]
		goal_state_shift_desired_goal["desired_goal"][:] = np.array(shift_desired_goals)[:]
		goal_state_shift_desired_goal = self._vec_normalize_env.normalize_obs(goal_state_shift_desired_goal)
		goal_state_shift_desired_goal["observation"] = th.from_numpy(goal_state_shift_desired_goal["observation"]).to(self.device)
		goal_state_shift_desired_goal["achieved_goal"] = th.from_numpy(goal_state_shift_desired_goal["achieved_goal"]).to(self.device)
		goal_state_shift_desired_goal["desired_goal"] = th.from_numpy(goal_state_shift_desired_goal["desired_goal"]).to(self.device)

		# Get next action using current policy
		base_next_actions = self.actor._predict(goal_state_shift_desired_goal, deterministic=True)

		# Compute the next Q values
		base_next_values = th.cat(self.critic_target.forward(goal_state_shift_desired_goal, base_next_actions), dim=1)
		base_next_values = th.min(base_next_values, dim=1, keepdim=True)[0].detach()
