...
class VecNormalize(VecEnvWrapper):
    ...
    def _normalize_obs(self, obs: np.ndarray, obs_rms: RunningMeanStd) -> np.ndarray:
        ...
        """
        if len(obs.shape) == 1:
            obs[:268] = np.clip((obs[:268] - obs_rms.mean[:268]) / np.sqrt(obs_rms.var[:268] + self.epsilon), -self.clip_obs, self.clip_obs)
        elif len(obs.shape) == 2:
            # print("obs.shape = ", obs.shape)
            # print("obs_rms.mean.shape = ", obs_rms.mean.shape)
            obs[:,:268] = np.clip((obs[:,:268] - obs_rms.mean[:268]) / np.sqrt(obs_rms.var[:268] + self.epsilon), -self.clip_obs, self.clip_obs)
        else:
            # print("obs.shape = ", obs.shape)
            # print("obs_rms.mean.shape = ", obs_rms.mean.shape)
    ...
    def _normalize_goal(self, obs: np.ndarray, obs_rms: RunningMeanStd) -> np.ndarray:
        """
        Helper to normalize observation.
        :param obs:
        :param obs_rms: associated statistics
        :return: normalized observation
        """
        if self.env_option == "grasping":
            if len(obs.shape) == 1:
                obs[:3] = np.clip((obs[:3] - obs_rms.mean[:3]) / np.sqrt(obs_rms.var[:3] + self.epsilon), -self.clip_obs, self.clip_obs)
            elif len(obs.shape) == 2:
                obs[:,:3] = np.clip((obs[:,:3] - obs_rms.mean[:3]) / np.sqrt(obs_rms.var[:3] + self.epsilon), -self.clip_obs, self.clip_obs)
            else:
                obs[:,:,:3] = np.clip((obs[:,:,:3] - obs_rms.mean[:3]) / np.sqrt(obs_rms.var[:3] + self.epsilon), -self.clip_obs, self.clip_obs)
            return obs
        elif self.env_option == "full":
            return np.clip((obs - obs_rms.mean) / np.sqrt(obs_rms.var + self.epsilon), -self.clip_obs, self.clip_obs)
        else:
            print("Unknown goal space definition")
            return
    ...
    def _unnormalize_obs(self, obs: np.ndarray, obs_rms: RunningMeanStd) -> np.ndarray:
        ...
        """
        if len(obs.shape) == 1:
            obs[:268] = (obs[:268] * np.sqrt(obs_rms.var[:268] + self.epsilon)) + obs_rms.mean[:268]
        elif len(obs.shape) == 2:
            obs[:,:268] = (obs[:,:268] * np.sqrt(obs_rms.var[:268] + self.epsilon)) + obs_rms.mean[:268]
        else:
            obs[:,:,:268] = (obs[:,:,:268] * np.sqrt(obs_rms.var[:268] + self.epsilon)) + obs_rms.mean[:268]
    ...
    def _unnormalize_obs(self, obs: np.ndarray, obs_rms: RunningMeanStd) -> np.ndarray:
        ...
        if self.env_option == "grasping":
            if len(obs.shape) == 1:
                obs[:3] = (obs[:3] * np.sqrt(obs_rms.var[:3] + self.epsilon)) + obs_rms.mean[:3]
            elif len(obs.shape) == 2:
                obs[:,:3] = (obs[:,:3] * np.sqrt(obs_rms.var[:3] + self.epsilon)) + obs_rms.mean[:3]
            else:
                obs[:,:,:3] = (obs[:,:,:3] * np.sqrt(obs_rms.var[:3] + self.epsilon)) + obs_rms.mean[:3]
        ...
        if self.norm_obs:
            if isinstance(obs, dict) and isinstance(self.obs_rms, dict):
                ...
                for key in self.norm_obs_keys:
                    ...
                    if "goal" in key:
                        obs_[key] = self._normalize_goal(obs[key], self.obs_rms[key]).astype(np.float32)
            else:
