...
class SAC(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        if self.make_logs:
            self.path = path
            self.f_log_next_q_values = open(self.path + "/log_next_q_values.txt", "w")
            self.f_log_target_q_values = open(self.path + "/log_target_q_values.txt", "w")
            self.f_next_log_prob = open(self.path + "/log_next_log_prob.txt", "w")
            self.f_log_current_q_values = open(self.path + "/log_current_q_values.txt", "w")
            self.f_weights_sum = open(self.path + "/log_weights_sum.txt", "w")

            ## save critic and actor losses for further analysis
            self.f_critic_losses = open(self.path + "/critic_losses.txt", "w")
            self.f_actor_losses = open(self.path + "/actor_losses.txt", "w")
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                target_q_values = (transformed_rewards*self.gamma + (1 - dones) * self.gamma * next_q_values).float()
    ...
    def _transform_rewards(self, replay_data, true_desired_goals, shift_desired_goals, her_indices, overshoot_goal):
        ...
        L_goals = self.env.envs[0].skill_manager.L_goals
