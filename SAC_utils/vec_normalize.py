...
class VecNormalize(VecEnvWrapper):
    ...
    def __init__(
        ...
        clip_obs: float = 10000.0,
        clip_reward: float = 10000.0,
        gamma: float = 0.99,
        epsilon: float = 1e-8,
    ):
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        ## add normalized desired and achieved goals to lookup table
        # self._update_lookup_tables(obs)
        ...
        for idx, done in enumerate(dones):
            ...
            if "terminal_observation" in infos[idx]:
                ...
                # print("infos[idx]['terminal_observation'] = ", infos[idx]["terminal_observation"])
                # self._update_lookup_tables(infos[idx]["terminal_observation"])

        self.ret[dones] = 0
        return obs, rewards, dones, infos

    def _update_lookup_tables(self, obs):
        """
        update relabelling lookup table and lookup table with normalized version
        of the achieved goals and the desired goals.
        """
        obs_ = deepcopy(obs)
        if obs["observation"].shape[0] == 1:
            self.envs[0]._add_to_lookup_table(obs_["achieved_goal"][0], obs_["desired_goal"][0])
        else:
            self.envs[0]._add_to_lookup_table(obs_["achieved_goal"], obs_["desired_goal"])

        shift_desired_goal_obs = deepcopy(obs_)

        ## shift desired goal of one indx in L_states
        if self.envs[0].tasks.indx_goal+1 < len(self.envs[0].tasks.L_states):
            # print("shift_desired_goal_obs['desired_goal'] = ", type(shift_desired_goal_obs["desired_goal"] ))
            shift_desired_goal_obs["desired_goal"] = np.array([self.envs[0].tasks.L_goals[self.envs[0].tasks.indx_goal+1]])

        ## associate normalized desired goal with next normalized desired goal
        if obs["observation"].shape[0] == 1:
            self.envs[0].tasks._add_to_lookup_table(obs_["desired_goal"][0], self.normalize_obs_key(shift_desired_goal_obs, "desired_goal")[0])
        else:
            self.envs[0].tasks._add_to_lookup_table(obs_["desired_goal"], self.normalize_obs_key(shift_desired_goal_obs, "desired_goal"))
    ...
    def _normalize_obs(self, obs: np.ndarray, obs_rms: RunningMeanStd) -> np.ndarray:
        ...
        return (obs - obs_rms.mean) / np.sqrt(obs_rms.var + self.epsilon)
    ...
    def normalize_obs_key(self, obs: Union[np.ndarray, Dict[str, np.ndarray]], key: str) -> Union[np.ndarray, Dict[str, np.ndarray]]:
    ...
    def reset(self) -> Union[np.ndarray, Dict[str, np.ndarray]]:
        ...
        # self._update_lookup_tables(norm_obs)
