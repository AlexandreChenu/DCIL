...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def _sample_transitions(
        ...
        ## get n-step returns (preliminary test)
        n_step_returns = np.zeros(self._buffer["reward"][episode_indices, transitions_indices].shape)
        for n_step in range(1,6):
            next_transitions_indices = (transitions_indices + n_step)%ep_lengths
            episode_indices_shift = (transitions_indices + n_step)//ep_lengths
            next_transitions_rewards = self._buffer["reward"][episode_indices + episode_indices_shift, next_transitions_indices]
            # print("next_transitions_rewards = ", next_transitions_rewards)
            n_step_returns += next_transitions_rewards
        n_step_observations = {"observation":self._buffer["observation"][episode_indices + episode_indices_shift, next_transitions_indices],
                                "achieved_goal":self._buffer["achieved_goal"][episode_indices + episode_indices_shift, next_transitions_indices],
                                "desired_goal":self._buffer["desired_goal"][episode_indices + episode_indices_shift, next_transitions_indices]}

        ## get next episode info for overshoot success boolean
        next_episode_indices = episode_indices.copy() + 1
        # print("episode_indices = ", episode_indices)
        # print("next_episode_indices = ", next_episode_indices)
        next_ep_lengths = self.episode_lengths[next_episode_indices]

        # print("next_ep_lengths = ", next_ep_lengths)
        ...
        next_ep_final_transitions_info = [
            ...
            for next_episode_idx, next_ep_length in zip(next_episode_indices,next_ep_lengths)
        ]

        # next_ep_final_transitions_info = []
        # for next_episode_idx, next_ep_length in zip(next_episode_indices,next_ep_lengths):
        #     print("next_episode_idx = ", next_episode_idx)
        #     print("next_ep_length = ", next_ep_length)
        #     next_ep_final_transitions_info.append(self.info_buffer[next_episode_idx][next_ep_length-1])
        #
        ...
        ## normalize n-step observation
        # print("n-step observations = ", n_step_observations)
        n_step_observations = self._normalize_obs(n_step_observations, maybe_vec_env)
        # print("norm n-step observations = ", n_step_observations)
        ...
        if online_sampling:
            ...
            ), transitions["info"], next_ep_final_transitions_info, her_indices, n_step_returns, n_step_observations
        else:
