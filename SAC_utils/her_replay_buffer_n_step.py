...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def __init__(
        ...
        self.n_step = n_step
        self.n_step_bonus = n_step_bonus
    ...
    def _sample_transitions(
        ...
        if online_sampling:
            ...
            if self.full:
                ...
                ## enable shift
                wrong_episode_indices_limit  = (episode_indices == self.n_episodes_stored-1).astype(int)

                # print("wrong_episode_indices = ", wrong_episode_indices_limit)
                episode_indices -= wrong_episode_indices_limit

                if self.n_episodes_stored != self.pos:
                    wrong_episode_indices_pos = (episode_indices == self.pos-1).astype(int)
                    episode_indices -= wrong_episode_indices_pos

                # print("episode_indices = ", episode_indices)
                    # np.random.randint(1,)
            else:
                episode_indices = np.random.randint(0, self.n_episodes_stored-1, batch_size)
        ...
        # print("norm next_observations = ", next_observations)


        ## n-step returns (preliminary test) - update
        n_steps = self.n_step
        n_step_transitions_indices = transitions_indices.copy()
        n_step_returns = np.zeros(self._buffer["reward"][episode_indices, n_step_transitions_indices].shape)
        n_step_shift = np.zeros(n_step_transitions_indices.shape)
        # print("\n\n\nep_lengths = ", ep_lengths[-10:])
        # print("transition_indices = ", transitions_indices[-10:])
        # print("\n")
        for i in range(1,n_steps+1):
            add_mask = (n_step_transitions_indices < ep_lengths-1) ## shift only if episode is not done
            # print("add_mask = ", add_mask[-10:])
            # print("n_step_transitions_indices = ", n_step_transitions_indices[-10:])
            # print("n_step_returns = ", n_step_returns[-10:])
            n_step_transitions_indices += add_mask
            # print("n_step_transition_indices = ", n_step_transitions_indices[-10:])
            # print("done = ", self._buffer["done"][episode_indices, n_step_transitions_indices][-10:])
            n_step_shift += add_mask
            n_step_returns += self._buffer["reward"][episode_indices, n_step_transitions_indices]*add_mask.reshape(n_step_returns.shape)
            # print("reward = ", self._buffer["reward"][episode_indices, n_step_transitions_indices][-10:].transpose() )

        # print("n_step_returns = ", n_step_returns[-10:].transpose())
        # get selected transitions
        n_step_transitions = {key: self._buffer[key][episode_indices, n_step_transitions_indices].copy() for key in self._buffer.keys()}

        # Convert info buffer to numpy array
        n_step_transitions["info"] = np.array(
            [
                self.info_buffer[episode_idx][transition_idx]
                for episode_idx, transition_idx in zip(episode_indices, n_step_transitions_indices)
            ]
        )

        # get next_obs for n_step transitions
        n_step_next_observations = {
            "observation": n_step_transitions["next_obs"],
            "achieved_goal": n_step_transitions["next_achieved_goal"],
            # The desired goal for the next observation must be the same as the previous one
            "desired_goal": n_step_transitions["desired_goal"],
        }
        # print("next_observations = ", next_observations)
        n_step_next_observations = self._normalize_obs(n_step_next_observations, maybe_vec_env)
        # print("norm next_observations = ", next_observations)

        ## n-step returns (preliminary test) - bonus reward -> only if n_step_next_observation is a success
        n_step_returns_bonus = np.zeros(self._buffer["reward"][episode_indices, n_step_transitions_indices].shape)
        next_ep_lengths = self.episode_lengths[episode_indices.copy()+1]
        # print("\nnext_ep_lengths = ", next_ep_lengths[-10:])
        n_step_transitions_indices_bonus = n_step_transitions_indices.copy()
        n_steps = self.n_step_bonus


        for n_step in range(1,n_steps+1):
            ## shift episode if reach the end of it
            episode_indices_shift = ((n_step_transitions_indices + n_step) > (ep_lengths-1)).astype(np.int64)
            # print("(n_step_transitions_indices + n_step) = ", (n_step_transitions_indices + n_step)[-10:])
            # print("episode_indices_shift = ", episode_indices_shift[-10:])
            ## if indices + n_step > ep_lenghts + next_ep_lengths -> stop to current step
            single_overshoot_mask = ((n_step_transitions_indices + n_step) <= (ep_lengths + next_ep_lengths - 1)).astype(np.int64)
            # print("single_overshoot_mask = ", single_overshoot_mask[-10:])

            n_step_transitions_indices_bonus = (n_step_transitions_indices + n_step - (ep_lengths)*episode_indices_shift)*single_overshoot_mask + (next_ep_lengths-1)*(1-single_overshoot_mask)
            # print("n_step_transitions_indices_bonus = ", n_step_transitions_indices_bonus[-10:])

            next_transitions_rewards = self._buffer["reward"][episode_indices + episode_indices_shift, n_step_transitions_indices_bonus]
            # print("next_transitions_rewards = ", next_transitions_rewards[-10:].transpose())
            # print("next_transitions_rewards = ", next_transitions_rewards)
            n_step_returns_bonus += next_transitions_rewards*single_overshoot_mask.reshape(next_transitions_rewards.shape)

        # print("n_step_returns_bonus = ", n_step_returns_bonus[-10:].transpose())

        n_step_transitions_bonus = {key: self._buffer[key][episode_indices + episode_indices_shift, n_step_transitions_indices_bonus].copy() for key in self._buffer.keys()}


        n_step_next_observations_bonus = {
            "observation": n_step_transitions_bonus["next_obs"],
            "achieved_goal": n_step_transitions_bonus["next_achieved_goal"],
            # The desired goal for the next observation must be the same as the previous one
            "desired_goal": n_step_transitions_bonus["desired_goal"],
        }
        n_step_next_observations_bonus = self._normalize_obs(n_step_next_observations_bonus, maybe_vec_env)

        ## get next episode info for overshoot success boolean
        next_episode_indices_bonus = episode_indices.copy() + 1
        next_ep_lengths = self.episode_lengths[next_episode_indices_bonus]

        #collect info from the last transition of the next episode (overshoot episode if success)
        next_ep_final_transitions_info = [
            self.info_buffer[next_episode_idx][next_ep_length-1]
            for next_episode_idx, next_ep_length in zip(next_episode_indices_bonus,next_ep_lengths)
        ]
        ...
        if online_sampling:
            ...
            n_step_next_obs = {key: self.to_torch(n_step_next_observations[key][:, 0, :]) for key in self._observation_keys}

            n_step_next_obs_bonus = {key: self.to_torch(n_step_next_observations_bonus[key][:, 0, :]) for key in self._observation_keys}
            ...
            # ), transitions["info"]
            # ), transitions["info"], n_step_returns, n_step_observations
            ), transitions["info"], next_ep_final_transitions_info, her_indices, n_step_returns_bonus, n_step_next_obs_bonus, n_step_transitions_bonus["done"], n_step_next_obs, self.to_torch(n_step_transitions["done"]), self.to_torch(self._normalize_reward(n_step_returns, maybe_vec_env)), n_step_shift
        else:
