...
class SAC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            replay_data, infos, next_ep_final_transitions_infos, her_indices  =  self.replay_buffer.sample(batch_size, env=self._vec_normalize_env)
            ...
            if all(self.env.envs[0].tasks.L_tasks_feasible) and self.add_bonus_reward:
            # if self.train_iteration > self.warmup_duration and self.add_bonus_reward:
                ...
                transformed_rewards = self._transform_rewards(replay_data,  next_desired_goals, her_indices, overshoot_goal)
    ...
    def _transform_rewards(self, replay_data, next_desired_goals, her_indices, overshoot_goal):
    ...
    def _transform_rewards_overshoot(self, replay_data, overshoot_results, her_indices, overshoot_goal):
        ...
        return rewards + th.from_numpy(bonus*relabelling_mask*goal_mask).float().to(self.device)
