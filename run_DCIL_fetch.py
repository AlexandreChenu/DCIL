...
def learn_DCIL(args, env, eval_env, path):
    ...
    callback = LogCallbackFetchEnv(path, args["algo_type"], eval_env)
...
if __name__ == '__main__':
    ...
    cur_path = os.getcwd()
    dir_path = cur_path + "/xp/GCPI_" + args["env_name"] + "_" + args["RL_algo"] + "_" + str(args["lr"]) + "_" + str(args["eps_optimizer"]) + "_" + str(args["demo_indx"]) + dt_string
    try:
        os.mkdir(dir_path)
    except OSError:
    ...
    assert len(env.skill_manager.starting_state_set)+1 == len(env.skill_manager.L_states)
    ...
    assert len(eval_env.skill_manager.starting_state_set)+1 == len(eval_env.skill_manager.L_states)
