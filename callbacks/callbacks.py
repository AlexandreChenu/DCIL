...
class LogCallbackSimpleMazeEnv(BaseCallback):
	"""
	...
	def __init__(self, path, algo, algo_type, env, verbose: int = 0):
		...
		if self.algo_type == "OffPolicyAlgorithm":
			...
			self.freq_rollout_display = 1000 #1000
	...
	def _visu_trajectories(self, env, eval_env, eval_trajs, training_trajs, success_trajs):
		...
		for indx, traj in enumerate(training_trajs):
			...
			ax.plot(X_traj, Y_traj, color = "lightsteelblue", marker = ".", zorder = 1)#, alpha = alpha ,zorder = 1)
		...
		circle = plt.Circle((L_X[len(L_X)-1], L_Y[len(L_X)-1]), eval_env.width_success, color='g', alpha = 0.2)
		circles.append(circle)
		# ax.add_patch(circle)
	...
	def _visu_V(self) -> None:
		...
		for state, goal_indx, desired_goal in zip(states, goal_indxs, desired_goals):
			self._visu_value_function(env, eval_env, state, goal_indx, desired_goal)
		return

	def _visu_value_function(self, env, eval_env, state, goal_indx, desired_goal):

		# s_v = np.linspace(0.5,1.,100)
		s_x = np.linspace(0.1, 2., 50)
		s_y = np.linspace(0.1, 2., 50)
		...
		fig.colorbar(cb)
		eval_env.draw(ax)
		ax.scatter(desired_goal[0],desired_goal[1])
...
class LogCallbackMazeEnv(BaseCallback):
	...
	def _on_step(self) -> bool:
		...
		# print("self.sum_W = ", self.sum_W)
		eval_env = self.locals["eval_env"]
		# print(self.locals["infos"])
		for info in self.locals["infos"]:
		#success = [info['target_reached'] for info in self.locals["infos"]]
			if info['done']:
				if True in info["successes"]:
					self.sum_W += 1
					self.success_trajs.append(info['traj'])
				self.n_runs += 1
				self.trajs.append(info['traj'])
	...
	def _visu_V(self) -> None:
		...
		## Toy DubinsMaze
		# states = [eval_env.skill_manager.L_states[0], eval_env.skill_manager.L_states[1]]
		states = [eval_env.skill_manager.L_states[0], eval_env.skill_manager.L_states[1], eval_env.skill_manager.L_states[10]]
		# goal_indxs = [1, 2]
		goal_indxs = [1, 2, 10]
		# desired_goals = [eval_env.skill_manager.L_states[1], eval_env.skill_manager.L_states[2]]
		desired_goals = [eval_env.skill_manager.L_states[1], eval_env.skill_manager.L_states[2], eval_env.skill_manager.L_states[11]]
		orientation_ranges = [[-np.pi/2,np.pi/2], [0.,np.pi], [-np.pi/2,np.pi/2]]
	...
	def _visu_value_function(self, env, eval_env, state, goal_indx, desired_goal):
		...
		min_x = (desired_goal[0] - 1.25)
		max_x = (desired_goal[0] + 1.25)

		min_y = (desired_goal[1] - 1.25)
		max_y = (desired_goal[1] + 1.25)
		...
		orientations = np.linspace(-np.pi/2., np.pi/2., 10)
		...
		for x, y in zip(states_x.flatten(), states_y.flatten()):
			...
			for theta in orientations:
				obs = eval_env.get_obs()
				#print("action = ", action)
				obs["observation"] = np.array([x, y, theta])
				obs["achieved_goal"] = np.array([x, y])
				obs["desired_goal"] = np.array([desired_goal[0],desired_goal[1]])
				...
				or_values.append(value.detach().cpu().numpy()[0])
		...
		circles = []
		for i in range(0,len(eval_env.skill_manager.L_states)):
			circle = plt.Circle((eval_env.skill_manager.L_states[i][0], eval_env.skill_manager.L_states[i][1]), eval_env.width_success, color='m', alpha = 0.6)
			circles.append(circle)
		...
		for i in range(1,len(eval_env.skill_manager.L_states)-1):
			x = eval_env.skill_manager.L_states[i][0]
			y = eval_env.skill_manager.L_states[i][1]
			dx = np.cos(eval_env.skill_manager.L_states[i][2])
			dy = np.sin(eval_env.skill_manager.L_states[i][2])
			arrow = plt.arrow(x,y,dx*0.3,dy*0.3,alpha = 0.2,width = 0.04, color="m", zorder=6)
			ax.add_patch(arrow)

		plt.savefig(self.path + "/visu_value_landscape_" + str(goal_indx) + "_it_" + str(self.nb_rollout) + ".png")
		plt.close(fig)
